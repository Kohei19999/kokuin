    ...
    for param2 in [60, 50, 40, 30, 20]:
        ...
        if circles is not None:
            for c in circles[0]:
                ...
                # 画像中心に近い円を優先する（背景のノイズ円を排除）
                dist_to_center = math.sqrt((cx - w/2)**2 + (cy - h/2)**2)
                score = 1.0 - (dist_to_center / (min_dim * 0.5))
                if score > best_score:
                    best_score = score
                    best_circle = (cx, cy, r)
            if best_circle and best_score > 0:
                break
...
def detect_hole(gray, coin_cx, coin_cy, coin_radius, debug_name=""):
    ...
    # ★色の違い検証: 穴候補の内部がコイン表面と明らかに違う色かどうかを確認
    # 「穴の内部の平均輝度」 vs 「穴の周囲リング（外側2倍半径）の平均輝度」を比較
    best_hole = None
    best_diff = 0  # 色の差が大きいほど「本物の穴」
    ...
    for hcx, hcy, hr, dist in candidates:
        ...
        # 白背景でも黒背景でも、少なくとも15ポイント以上の差があれば「物理的な穴（背景が見えている）」とみなす
        if color_diff > 15 and color_diff > best_diff:
