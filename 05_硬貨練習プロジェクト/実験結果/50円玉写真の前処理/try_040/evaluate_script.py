...
def pipeline_A_check_hole_presence(img, base_filename, preproc_out_dir):
    ...
    # 元ROIの保存
    cv2_imwrite_jp(os.path.join(preproc_out_dir, f"{base_filename}_pipelineA_1_roi_original.jpg"), roi)
    
    # 【try_004 新規】割算による照明平坦化（Division by Gaussian Background）
    # WEB調査で発見した手法。巨大カーネルでぼかして「照明ムラだけ」を抽出し、
    # 元画像を割算することで照明の影響を完全に相殺する。
    # これにより、光が当たって穴が明るく見える問題（003, 010, 013, 014の失敗原因）を解消。
    roi_float = roi.astype(np.float32)
    background = cv2.GaussianBlur(roi_float, (51, 51), 0)
    # 割算（照明ムラを相殺）+ スケール255で正規化
    normalized = cv2.divide(roi_float, background, scale=255)
    roi_normalized = np.clip(normalized, 0, 255).astype(np.uint8)
    
    cv2_imwrite_jp(os.path.join(preproc_out_dir, f"{base_filename}_pipelineA_2_illumination_normalized.jpg"), roi_normalized)
    
    # 照明平坦化後の画像に対して絶対閾値で二値化
    # 平坦化により穴は常に暗く浮き出るため、閾値を少し緩和（50→200）できる
    # ※ 平坦化後は「均一な灰色（〜255）の背景に暗い穴」になるため、閾値の意味が変わる
    _, thresh = cv2.threshold(roi_normalized, 200, 255, cv2.THRESH_BINARY_INV)
    ...
    cv2_imwrite_jp(os.path.join(preproc_out_dir, f"{base_filename}_pipelineA_3_thresh.jpg"), thresh)
