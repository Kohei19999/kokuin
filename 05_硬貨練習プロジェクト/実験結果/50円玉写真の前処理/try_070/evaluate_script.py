...
def pipeline_A_check_hole_presence(img, coin_cx, coin_cy, coin_radius, base_filename, preproc_out_dir):
    ...
    # 相対処理（Otsuや平坦化、ブラックハット）は「穴がないコイン（011,012）」で偽の穴を
    # 作り出してしまうため一切使わない。貫通穴の「絶対的な暗さ」だけを信じる（固定閾値: 60）
    ...
    cv2_imwrite_jp(os.path.join(preproc_out_dir, f"{base_filename}_A2_thresh.jpg"), thresh)
    
    hole_pixels = cv2.countNonZero(thresh)
    
    # ピクセル数が一定以上なら穴あり
    is_hole = 1 if hole_pixels > 150 else 0
