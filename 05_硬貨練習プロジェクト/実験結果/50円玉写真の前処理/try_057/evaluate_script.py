...
def find_coin_center(img, base_filename, preproc_out_dir):
    ...
    coin_radius = min(img.shape) // 4
    
    # 輪郭（エッジ）ベースの中心検出は、背景の影などを硬貨の縁と誤認すると
    # コイン自体の中心座標がズレてしまい、結果的に穴のズレ判定（相対距離）が狂ってしまう。
    # そこで、より真円の検出に特化した「ハフ変換（HoughCircles）」を使用する！
    ...
    # ハフ変換で円が見つかった場合はその中心と半径を採用
    if circles is not None and len(circles) > 0:
        c = circles[0][0]
        coin_cx, coin_cy, coin_radius = int(c[0]), int(c[1]), int(c[2])
    else:
        # 見つからなかった場合のフェールセーフ（一応残す）
        clahe = cv2.createCLAHE(clipLimit=2.5, tileGridSize=(8,8))
        enhanced = clahe.apply(img)
        edges = cv2.Canny(cv2.medianBlur(enhanced, 9), 30, 100)
        ...
        if valid_contours:
            ...
            M = cv2.moments(best)
            if M["m00"] != 0:
                coin_cx = int(M["m10"] / M["m00"])
                coin_cy = int(M["m01"] / M["m00"])
            else:
                (x, y), _ = cv2.minEnclosingCircle(best)
                coin_cx, coin_cy = int(x), int(y)
            coin_radius = int(math.sqrt(cv2.contourArea(best) / math.pi))
...
def pipeline_B_check_misalignment(img, coin_cx, coin_cy, coin_radius, base_filename, preproc_out_dir):
    ...
    # パイプラインAで100%成功した「最強の穴抽出マスク」をここでも構築する
    # ※ここでroi_sizeを広くしすぎると硬貨の内周の黒枠を拾ってしまい、外接円の中心が硬貨中心と一致してしまうので注意！
